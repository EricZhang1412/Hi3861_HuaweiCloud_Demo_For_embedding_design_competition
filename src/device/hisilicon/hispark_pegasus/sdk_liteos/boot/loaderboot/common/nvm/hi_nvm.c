...
hi_u32 nv_init_common(hi_u32 base_addr, hi_u32 total_size, hi_u32 block_size, hi_nv_type nv_type)
{
    ...
    for (hi_u16 i = 0; i < (hi_u16)total_size / (hi_u16)block_size; i++) {
        ...
        if (nv_read_flash(flash_addr, block_size, data) != HI_ERR_SUCCESS) {
            /* Continue to process read errors. */
            continue;
        }
        if (nv_check_file(data, block_size, nv_type) != HI_ERR_SUCCESS) { /* Check the validity of the NV file. */
            continue;
        }
        hi_nv_manage* nv_head = (hi_nv_manage*)data;
    ...
    /* Applying for Index Space CNcomment:索引空间申请 */
...
hi_u32 nv_common_write(hi_u8 id, hi_nv_type nv_type, hi_u8 len, const hi_pvoid data)
{
    ...
    if (nv_type == HI_TYPE_FACTORY_NV) {
        backup_num = 2; /* 2:Two copies of operations in the factory area are backed up. */
    }
